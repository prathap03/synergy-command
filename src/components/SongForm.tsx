...
export default function SongForm({ initial, onSave, onCancel }: Props) {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (!initial) setForm({ clue: '', answer: '', movie: '', director_singer: '', level: 1 })
  }
  ...
  return (
    <form onSubmit={submit} className="flex flex-col gap-3">
      ...
      <select
        ...
        className="input-field"
      >
        <option value={1}>🎵 Round 1 — Nostalgia Hits</option>
        <option value={2}>🔥 Round 2 — Modern Kuthu</option>
        <option value={3}>🤪 Round 3 — Absurd Literalism</option>
      </select>
