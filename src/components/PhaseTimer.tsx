...
export default function PhaseTimer({ durationSec, storageKey, label, fullscreen = false, onDone }: Props) {
  const { remaining, running, start, pause, reset, pct, done } = useTimer(durationSec, storageKey)
  ...
  if (fullscreen) {
    return (
      <div className="flex flex-col items-center justify-center gap-4">
        ...
        <div className={`text-8xl font-black tabular-nums transition-colors ${done ? 'animate-pulse' : ''}`} style={{ color }}>
  ...
  return (
    <div className="flex items-center gap-2 bg-white/10 rounded-xl px-3 py-2">
      ...
      <span className={`font-mono font-bold tabular-nums text-lg transition-colors ${done ? 'animate-pulse' : ''}`} style={{ color }}>
        {formatTime(remaining)}
      </span>
      {!running
